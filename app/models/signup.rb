class Signup < ActiveRecord::Base
  ...
  validates :membership_no, :presence => true, :length => { :is => 7 }
...
# validates :application_no, :presence => true, :uniqueness => true, :length => { :maximum => 10 }
  ...
  def membership_no_should_be_unique
    ...
    unless signup.nil?
      errors.add(:membership_no, "already used")
    end
  end
  ...
  def company_and_employee_not_blank
      ...
      if employee_no.blank?
        errors.add(:employee_no, "can't be blank for corporate plan!") if plan.category == "C"
      end

      if company_id.blank?
        errors.add(:company_id, "name can't be blank for corporate plan!") if plan.category == "C"
      end
  end
  ...
  def company_and_employee_uniqness
        plan = Plan.find(plan_id)
          signup = Signup.find(:all, :conditions=> ["company_id=? and employee_no=?", company_id, employee_no])
           unless signup.blank?
           errors.add(:company_id, " and employee is already a member!") if plan.category == "C"
           end
  end
  ...
  def referrer_existence_validation
   ...
   unless referrer_member_id.blank?
      card= Card.find_all_by_membership_no(referrer_member_id)
       if card.blank?
        errors.add(:referrer_member_id, " is not valid!") if plan.category == "C"
       end
  end
 end
